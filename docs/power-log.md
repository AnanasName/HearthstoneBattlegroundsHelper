...
Лечится ключом **`TruncatePos`** (предел в КБ, по умолчанию 10000) в каждой секции.
Плюс отключением ненужных секций, чтобы никакой файл не подходил к пределу.
Действующий конфиг: только `[Power]` и `[LoadingScreen]`, `TruncatePos=200000`.

Практическое следствие для продукта, а не только для фикстур: при настройках по умолчанию
любой ридер логов слепнет примерно через 10 минут игры. Инструкция по установке обязана
менять `TruncatePos`, иначе помощник будет работать только первые пару боёв.
