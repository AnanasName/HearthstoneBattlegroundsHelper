...
## CHANGE_ENTITY: сущность стала ДРУГОЙ картой (part46)

Четвёртый вид строки про сущность рядом с `FULL_ENTITY`, `SHOW_ENTITY`
и `HIDE_ENTITY`. Формы две, обе как у `SHOW_ENTITY`:

```
CHANGE_ENTITY - Updating Entity=[entityName=Зирелла id=115 zone=HAND zonePos=2 cardId=BG20_HERO_101 player=1] CardID=BG26_HERO_102
CHANGE_ENTITY - Updating Entity=6544 CardID=BG31_843_G
```

Читать надо ХВОСТ: дескриптор по общему правилу (ловушка 5) показывает
состояние ДО замены, то есть СТАРУЮ карту, а новая стоит после `CardID=`.
За строкой идёт блок тегов новой карты — `CARDTYPE`, `ATK`, `HEALTH`,
`PREMIUM`, `CARDRACE`, `NUM_TURNS_IN_HAND`, — поэтому строку обязан узнавать
и тот, кому сам cardId не нужен: пока она не узнавалась, «текущая сущность»
оставалась ПРЕЖНЕЙ, и весь этот блок тегов уходил к ней.

Счёт по 45 фикстурам: **252 строки в канале-источнике**, от 1 до 42 на партию.
Делают они три разные вещи:

| что | сколько | зона | пример |
|---|---|---|---|
| слот аксессуара становится взятым тринкетом | по 2 на партию | `PLAY`, наш игрок | `BG30_Trinket_1st` → `BG30_MagicItem_924` |
| «Сейф» в руке открывается ЗОЛОТЫМ миньоном | 16 за part46 | `HAND`, наш игрок | `BG36_520t` → `BG33_140_G` |
| миньон соперника превращается в бою | единицы | `PLAY`, чужой игрок | `BG34_858` → … |
| замена героя ЖЕТОНОМ на экране выбора | 1 за part46 | `HAND`, наш игрок | `BG20_HERO_101` → `BG26_HERO_102` |

Последняя строка — это кнопка «Заменить» с монеткой на экране выбора героя
(part46, 23:04:55). Сущность варианта остаётся ТОЙ ЖЕ (`id=115`), меняется
только карта, и через шесть секунд `SendChoices` называет уже нового героя.
Значит вариант модального выбора — это СУЩНОСТЬ, а не строка `Entities[i]=`,
которой канал выбора назвал её при открытии: строки приходят один раз,
а карта под ними меняется.

Сейф `BG36_520t` («Unplayable. In 5 turns, break this open and get a random
Golden minion with a type!») создаётся как `CARDTYPE=SPELL`, а при открытии
получает `CARDTYPE=MINION` вместе со статами — то есть карта меняет и ТИП,
и зону не меняет вовсе.
...
## Режим партии: `GameType` — единственный способ отличить чужую игру

Строка канала метаданных, сразу после CREATE_GAME, рядом с `BuildNumber`:

```
D 19:21:20.1976107 GameState.DebugPrintGame() - GameType=GT_RANKED
D 15:14:59.3503624 GameState.DebugPrintGame() - GameType=GT_BATTLEGROUNDS
```

Читать её обязательно, и вот почему: **обычная партия Hearthstone
складывает наши признаки таверны САМА, и складывает ложно.** Замер
на живом логе игрока (05.09, сессия 19:16 — шесть `GT_RANKED` и ни одной
Battlegrounds, 19.7 МБ):

- фаза читается `tavern` на 12 067 событиях;
- «золото» идёт мановой кривой — 5/5, 6/6, 7/7, 8/8, 9/9, 10/10;
- в «витрину» попадают чужие карты (`CATA_722(5/4)`, `CATA_580t(2/1)`);
- тир везде 1, таблица лобби пуста, места нет.

Итог до правки: пакетный путь давал **шесть точек решения**, а живой
рекордер записывал такую партию в датасет как настоящую партию BG.
Ни один косвенный признак тут не помогает — пустое лобби бывает и у BG
в начале, а тир 1 законен на первом ходу; отличает только сам тег.

Парсер пропускает из этого канала ровно две строки — `BuildNumber`
и `GameType`; всё остальное там либо читается отдельно (имена игроков),
либо не нужно. Неизвестный режим (`gameType: null`) считается СВОИМ:
у сегмента переподключения строки нет вовсе (part1, part35, part41),
и выбрасывать такие партии значило бы терять доигранные ради страховки
от режима, которого в них не бывает.
...
## Удвоение примагничивания: безымянный тег 4945 (part44)

Дрон-дубликатор `BG36_506` (тир 4): «**Activate ({0}): The next
Magnetization to this minion this turn is doubled**»; у золотой копии
`BG36_506_G` — «tripled». Факт «режим включён СЕЙЧАС» игра сообщает тегом,
у которого нет имени — в логе он числом, как связка `2442` между кнопкой
покупки и миньоном витрины (part35).

Ставится он внутри блока `PLAY` самой активации, на СВОЮ же сущность:

```
13:15:55 BLOCK_START BlockType=PLAY Entity=[Дрон-дубликатор id=7756 zone=PLAY zonePos=2 cardId=BG36_506]
13:15:55     TAG_CHANGE Entity=AngryMem#2886 tag=RESOURCES_USED value=6
13:15:55     BLOCK_START BlockType=POWER Entity=[… id=7756 …]
13:15:55         SUB_SPELL_START … Source=7756 TargetCount=1
13:15:55                           Targets[0] = [… id=7756 …]
13:15:55             TAG_CHANGE Entity=[… id=7756 …] tag=4945 value=1     ← режим включён
13:15:55         TAG_CHANGE Entity=[… id=7756 …] tag=INTERACTABLE_OBJECT value=0
```

и гаснет ТЕМ САМЫМ примагничиванием, которое удвоил, — внутри блока
`PLAY` модуля:

```
13:16:33 (блок розыгрыша Щита Легиона)
             TAG_CHANGE Entity=[… id=7756 …] tag=4945 value=0            ← режим потрачен
```

За part44 таких пар РОВНО ПЯТЬ, все на Дрон-дубликаторе и все внутри
своего хода: 13:12:07 → 13:12:11 (#4628), 13:13:33 → 13:13:39 (#4977),
13:15:55 → 13:16:33 (#7756), 13:15:56 → 13:16:04 (#6598), 13:18:37 →
13:18:38 (#8666). Ни одного «включилось и не погасло» и ни одного
на другой карте.

Читать надо ИМЕННО тег, а не «нажата ли активация в этом ходу»:
на ходу 17 игрок нажал активацию на ОБОИХ дубликаторах, но у #6598 режим
к моменту кадра уже потрачен магнитом Clunker Junker, а у #7756 ещё цел.
По `activatedEntityIds` эти два состояния неразличимы.

Множитель тегом НЕ сообщается — его называет ТЕКСТ карты
(«doubled»/«tripled»), и у золотой копии слово своё. Подставлять тройку
по признаку `PREMIUM` значило бы гадать там, где карта говорит прямо.

## Сила, поднимающая карту ВИТРИНЫ на тир выше (part47)

«Алчность Галакронда» `TB_BaconShop_HP_011`, цена 1. Нажатие видно
обычным блоком PLAY на сущности силы, но ЦЕЛЬ у него необычная — миньон
витрины, а не наш борд:

```
BLOCK_START BlockType=PLAY Entity=[… id=167 … cardId=TB_BaconShop_HP_011 player=8]
    … Target=[entityName=Раскаленный валун id=766 … cardId=BGS_127 player=16]
```

`player=16` — это Бармен Боб, то есть витрина. Дальше внутри блока игра
создаёт варианты в `SETASIDE` (`FULL_ENTITY` с `CHOOSE_ONE=1`,
`CREATOR` = id силы) и открывает обычный канал выбора:

```
DebugPrintEntityChoices() - id=2 … ChoiceType=GENERAL CountMin=1 CountMax=1
  Source=[… cardId=TB_BaconShop_HP_011 player=8]
  Entities[0..2]=… (три миньона)
```

**Тир вариантов = тир ЦЕЛИ плюс один**, и это проверено всеми пятью
нажатиями партии: цели тира 1, 2, 3, 4, 5 дали варианты тира 2, 3, 4, 5, 6
(`TECH_LEVEL` на созданных сущностях). Тир таверны на предложение
не влияет — на ходу 11 при таверне 2 предложение было тира 6.

После `SendChoices` сущность-вариант уходит в `REMOVEDFROMGAME` с обнулением
тегов (это была лишь заготовка выбора), сама ЦЕЛЬ тоже уходит
в `REMOVEDFROMGAME`, а в витрину встаёт НОВАЯ сущность выбранной карты
(`SHOW_ENTITY`). То есть выбранный миньон не приходит в руку и не встаёт
на борд: он занимает место цели в витрине, и купить его надо отдельно
за обычную цену.

Такая сила в пуле одна: слова «higher Tier» среди 169 сил набора
Battlegrounds есть только у неё (и у одной карты из 35 714 во всём
снапшоте).
...
## Сила, которую жмут ДВАЖДЫ за ход: `EXHAUSTED` и счётчик активаций (part45)

Уточнение к записи part32 («`EXHAUSTED` — подтверждение, а не источник»).
После part45 у тега появилась работа, которой не делает никто другой:
он единственный отличает «нажал один раз из двух» от «нажал и всё».

**Фактура.** Инге Стальной Гимн, сила `BG26_HERO_102p` (Major Hymn)
и `BG26_HERO_102p2` (Minor Hymn) — половины меняются местами каждый ход,
и сущность силы при этом НОВАЯ (id 167 → 749 → 1130 → 1716 → 2513 → …).
Цены у силы нет вовсе (тега `COST` не приходит), `HAS_ACTIVATE_POWER=1`
с первого хода, `LOCK_VISUAL` не приходит ни разу.

Каждый ход в логе стоит одинаковая тройка событий:

```
15:27:59.3876113  BLOCK_START BlockType=PLAY  Entity=[... id=167 cardId=BG26_HERO_102p]
15:27:59.3876113      TAG_CHANGE Entity=AngryMem#2886 tag=HEROPOWER_ACTIVATIONS_THIS_TURN value=1
15:27:59.3876113      TAG_CHANGE Entity=[... id=167 ...] tag=EXHAUSTED value=1
15:27:59.3876113      TAG_CHANGE Entity=[... id=167 ...] tag=EXHAUSTED value=0     ← ВОЗВРАТ
15:28:04.6717541  BLOCK_START BlockType=PLAY  Entity=[... id=167 ...]              ← второе нажатие
15:28:04.6717541      TAG_CHANGE Entity=AngryMem#2886 tag=HEROPOWER_ACTIVATIONS_THIS_TURN value=2
15:28:04.6717541      TAG_CHANGE Entity=[... id=167 ...] tag=EXHAUSTED value=1     ← остаётся до конца хода
```

Ключевое — **возврат `1 → 0` стоит на той же отметке времени, внутри
блока самого нажатия**, отдельным триггером его не сопровождают.

**Чем это отличается от part29.** У Тавиша (`BG22_HERO_000p_Alt`) тег
тоже есть, но `EXHAUSTED=0` приходит РОВНО СО СМЕНОЙ ХОДА — все четыре
раза (01:09:52, 01:10:40, 01:11:42, 01:13:30 против нажатий в 01:09:37,
01:10:22, 01:11:20, 01:12:41). То есть у обычной силы состояния «нажата
в этом ходу, но не исчерпана» не существует, а у Инге оно и есть
нормальное состояние между двумя нажатиями.

**Сколько нажатий положено — в логе НЕ НАПИСАНО.** Счётчик
`HEROPOWER_ACTIVATIONS_THIS_TURN` (приходит и на игрока, и на сущность
силы) за партию принимает значения 0, 1 и 2 и НИ РАЗУ 3, а третье
нажатие запрещает канал опций:
`error=REQ_NOT_EXHAUSTED_HERO_POWER` (246 строк за партию против 238
со `error=NONE`). Но самого лимита как числа нет ни в одном теге и ни
в тексте карты — при создании силы стоят только `BACON_HEROPOWER_BASE_HERO_ID`,
`HAS_ACTIVATE_POWER` и безымянный `4901`. Поэтому читать надо ФАКТ
«можно ли нажать сейчас» (`EXHAUSTED`), а не выводить лимит.


## Сила «ОДИН РАЗ ЗА ПАРТИЮ»: `HERO_POWER_DISABLED` (part48)

«Нас ждёт богатство!» Рено `TB_BaconShop_HP_046` («Once per game, make
a friendly minion Golden») жмётся один раз за партию, и по прежним тегам
это НЕ ВИДНО. Нажатие 13:29:04 (`BLOCK_START BlockType=PLAY` на сущности
силы, `CARD_TARGET` — id миньона) ставит подряд:

```
tag=HERO_POWER_DISABLED value=1
tag=EXHAUSTED value=1
tag=HEROPOWER_ACTIVATIONS_THIS_TURN value=1
```

А в начале следующего хода (13:29:46) игра снимает ДВА из трёх:
`EXHAUSTED value=0`, `HEROPOWER_ACTIVATIONS_THIS_TURN value=0`, — и
`HERO_POWER_DISABLED` остаётся единицей до конца партии.
`LITERALLY_UNPLAYABLE` на этой силе не приходит ни разу. То есть без
чтения этого тега потраченная сила по всем признакам выглядит готовой.

**Тег значит «сейчас нельзя», а не «потрачена навсегда».** По 48
фикстурам он встречается у четырёх сил, и у двух возвращается в ноль:
«Три желания» `TB_BaconShop_HP_102` («If you have two copies of a minion,
find the third») в part15 и «Бранное дело» `TB_BaconShop_HP_048` в part34
— там единица приходит ровно тогда, когда отработала награда за четыре
кличевые покупки. Ещё одна — «Cloning Gallery» `BG31_HERO_005p` («Once
per game, summon an exact copy of a friendly minion»), у которой
в фикстурах встречались только нули.

### Что делает превращение в золото

`CHANGE_ENTITY` на цели (`CardID=BG31_809_G`) плюс пара тегов: Бирюзовый
быстролап 35/14 становится 40/19 при базовых 5/5 и золотых 10/10.
Значит прибавка — это РАЗНИЦА БАЗОВЫХ статов, а накопленные усиления
остаются на месте. По снапшоту золотая копия удваивает статы у 377 карт
пула из 379 (исключения — Aureate Laureate 2/2 → 2/2 и Sky-hatch Runaway
4/7 → 10/14), поэтому числа надо брать с карты, а не удваивать.

## Кровавый самоцвет: размер лежит у ИГРОКА (part48)

Сам самоцвет `BG20_GEM` — карта с текстом «Give a minion +{0}/+{1}»
и плейсхолдерами на своей сущности: у игрока без надбавок это 1 и 1
(13:12:19). Надбавку игра держит на сущности ИГРОКА тегами
`BACON_BLOODGEMBUFFATKVALUE` и `BACON_BLOODGEMBUFFHEALTHVALUE` — в part48
у соперника они доходят до восьми, в part36 у нас до двенадцати. Пара
устроена как элементальная (`BACON_ELEMENTAL_BUFF*`) и читается тем же
путём — только от СВОЕГО игрока.

**Племени у цели нет.** Текст «Give a minion» надо читать буквально:
