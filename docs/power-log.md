...
Уточнение part52: на сущности **«Бармен Боб»** (id=44) тот же тег называет
как раз СОПЕРНИКА (`value=4` в 00:58:00, `value=5` в 01:00:36) — негоден
он на сущности игрока. Опираться на это не стали: героя читать надёжнее.

⚠️ **Порядок строк `HERO_ENTITY` и `PLAYER_ID` не закреплён, и в бою-РЕВАНШЕ
он обратный** (part52, D215). Обычный бой — сначала герой объявляет себя,
потом слот берёт героя:

```
TAG_CHANGE Entity=554 tag=PLAYER_ID value=3            <- 00:37:08.9474589
TAG_CHANGE Entity=Dumdidum tag=HERO_ENTITY value=554
```

Реванш с тем, с кем уже дрались, — наоборот:

```
TAG_CHANGE Entity=Бармен Боб tag=HERO_ENTITY value=9565   <- 00:50:14.2456984
TAG_CHANGE Entity=9565 tag=PLAYER_ID value=7
```

В part52 таких боёв два из тринадцати (ходы 18 и 24). Разбор, который ищет
владельца героя только в момент `HERO_ENTITY`, на них промахивается МОЛЧА
и оставляет соперника от прошлого боя — борд записывается чужому игроку.
Читать надо оба порядка: запомнить id героя чужого слота и поставить
владельца, когда придёт его строка.
...
**Второй, не связанный путь к той же цене — `BACON_OVERRIDE_BG_COST`
на самом миньоне витрины, и это АБСОЛЮТНОЕ значение, а не дельта**
(part70, 23.09, Синдрагоса: сила `TB_BaconShop_HP_014` «Stay Frosty»,
«Minions cost (2)»). В этой партии `BACON_REDUCE_BUY_COST`
и `BACON_SHOW_OVERRIDEN_MINION_COST` не встречаются НИ РАЗУ, то есть
пара выше — не единственная форма скидки и по её отсутствию судить
о цене нельзя:

    TAG_CHANGE Entity=394 tag=BACON_OVERRIDE_BG_COST value=2

Счёт по партии: 480 строк — 240 со значением 2 на живых миньонах
витрины и 240 со значением 0 при уходе из неё. Кнопка `DragBuy` тех же
миньонов несёт `COST=2` синхронно, все 120 штук за партию, — поэтому
цена в советах партии была верна: мы читаем кнопку, а не скидку
(см. ниже). Вывод тот же, что в part35, и подтверждён вторым способом:
источник цены — КНОПКА, прочие теги описывают, откуда цена взялась.

**Автоматическая заморозка витрины — блок `TRIGGER` на сущности силы
героя** (part70, та же Синдрагоса, «The Tavern … Freezes at the end
of each turn»). Не тег на игроке и не отдельное событие витрины:

    BLOCK_START BlockType=TRIGGER Entity=[entityName=Бодрящий мороз id=196
      zone=PLAY zonePos=0 cardId=TB_BaconShop_HP_014 player=1]
      EffectCardId=… EffectIndex=0 Target=0 SubOption=-1 TriggerKeyword=2882
    TAG_CHANGE Entity=[… id=1554 … player=9] tag=FROZEN value=1

11 срабатываний на 11 ходов таверны, включая ход, в котором игрок
не купил ничего (там помечены все четыре слота витрины подряд).
**Но до следующей точки решения заморозка не доживает:** тем же блоком,
на смене хода, приходит `FROZEN value=0`. То есть по состоянию таверны
такой герой неотличим от обычного, и единственный источник знания
о его заморозке — текст силы (**D282**).
...
## Цена силы ДОХОДИТ ДО НУЛЯ, а обещанная карта написана на силе (part53)

«Галактическая линза» Нобундо `BG31_HERO_003p`: «Get a copy of the last
Tavern spell you cast. Each turn, your next Hero Power costs (1) less».
Сущность силы одна на всю партию (`id=211`), и на ней живут ДВА факта,
которых нет больше нигде.

**Цена — тег `COST`, и ноль у него значащий.** Канал-источник, по порядку:

```
01:08:21 … id=211 … tag=COST value=2
01:09:01 … id=211 … tag=COST value=1
01:09:58 … id=211 … tag=COST value=0
01:10:28 BLOCK_START BlockType=PLAY Entity=[entityName=Галактическая линза id=211 … player=8]
01:10:28 … id=211 … tag=COST value=3
```

Лестница 3 → 2 → 1 → 0 идёт по ходам таверны, нажатие сбрасывает её в 3.
Нулей за партию два (01:09:58, 01:14:36). Тег `COST` при этом ЕСТЬ —
это отличает скидку до нуля от бесплатной по устройству силы, у которой
тега нет вовсе (part13, D083).

На ТОЧКЕ РЕШЕНИЯ ноль не виден ни разу: нажатие за ноль не растит счётчик
трат (`RESOURCES_USED`), и точка решения (последнее состояние до первой
траты) съезжает за нажатие — там сила уже `EXHAUSTED=1` с ценой 3.

**Обещанная карта — тег `BACON_EVOLUTION_CARD_ID` на той же сущности.**
Значение — dbfId заклинания таверны; последнее значение перед нажатием
совпало с картой, созданной нажатием в `HAND`, шесть раз из шести:

```
01:07:55 … id=211 … tag=BACON_EVOLUTION_CARD_ID value=104436   (Tavern Coin)
01:10:28 BLOCK_START BlockType=PLAY … id=211 …
01:10:28     FULL_ENTITY - Creating ID=1809 CardID=BG28_810
                 tag=CARDTYPE value=SPELL
                 tag=ZONE value=HAND
                 tag=CREATOR value=211
```

Прочие пары: 105664 → `BG28_518` (01:15:05), 104436 → `BG28_810`
(01:20:58), 104029 → `BG28_805` (01:25:10, 01:26:30, 01:29:27). Тег
меняется каждым розыгрышем заклинания таверны — с 01:23:03 по 01:23:29
восемь смен за 26 секунд, — то есть читать надо ЖИВОЕ значение, а не
первое за ход.

**На силах героя тег значит одно и то же — «карта, которую сила выдаст
или в которую превратится».** Скан канала-источника по part4–part53 нашёл
его на пяти силах, и на каждой значение — dbfId этой карты, а `0` —
«пока нечего»:

| сила | фикстура | значение → карта |
|---|---|---|
| «Галактическая линза» `BG31_HERO_003p` | part53 (и part17, 22, 37) | заклинание, которое скопирует, 6 из 6 |
| «Battle Brand» `TB_BaconShop_HP_048` | part14, 22 | 96786 → `BG_LOE_077` Brann Bronzebeard — награда |
| «BUY, INSECT!» `TB_BaconShop_HP_087` | part13, 33, 38, 49 | 64426 → `TB_BaconShop_HP_087t` Sulfuras — награда |
| «Lock and Load» `BG22_HERO_000p_Alt` | part26, 29 | заряженный миньон (Leeroy, Prisonguard, Tusked Camper) |
| «Azshara's Ambition» `BG22_HERO_007p` | part30 | 80007 → `BG22_HERO_007p2` Naga Conquest — во что превратится |

Сверка «обещано → выдано» сделана только на Линзе. На остальных четырёх
значение совпадает с текстом силы по смыслу, но нажатие с выдачей
построчно не сверялось.

**На МИНЬОНАХ смысл тега другой.** В part52 он стоит на миньонах витрины
(Auto Assembler `BG32_172` — 34 строки, `BG35_123` — 32), и «обещанной
