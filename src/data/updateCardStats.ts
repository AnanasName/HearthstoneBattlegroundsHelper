 ...
 *
 * ## Это ИНСТРУМЕНТ ЗАМЕРА, а не часть рантайма
 *
 * Замер 15.08 (docs/cardstats.md) пользы от этих данных в оценке покупки
 * не показал: сигнала сверх наших одиннадцати слагаемых нет (V2 = −0.078
 * при пороге +0.15, p = 0.87). Снапшот поэтому НЕ коммитится и советником
 * не читается; команда живёт, чтобы замер можно было повторить на новых
 * партиях своего билда, когда их накопится заметно больше тринадцати.
 *
