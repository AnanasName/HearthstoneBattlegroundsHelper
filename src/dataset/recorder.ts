...
export interface DatasetRecord {
  ...
  /**
   * Номер фикстуры, чей лог и есть эта партия. Ставит досбор (19.09): живая
   * запись имени партии не знает, а отпечаток её не находит, когда запись
   * сделана старым редьюсером (герой до подмены D200, место до D235) или
   * обрывком после перезапуска клиента. По этому полю загрузчик оставляет
   * одну запись на партию.
   */
  readonly fixturePart?: number;
}
