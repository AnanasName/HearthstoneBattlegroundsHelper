 ...
 * дошёл до part35: **part28** — оверлей включён посреди партии, живая
 * запись начинается с хода 3 (8 точек) против хода 1 у досбора (9);
 * **part35** — клиент перезапускался, и живой путь записал только хвост
 * после реконнекта (3 точки с хода 21 против 12 с хода 1). Отпечатки
 * разошлись, обе партии легли в датасет дважды. Досбор теперь такие пары
 * НАЗЫВАЕТ (`backfill.ts`): прежде предупреждение молчало, если у лежащей
 * записи есть `lobby`, — а у живых записей 26–28.08 он есть. Что делать
 * с парой, по-прежнему решает владелец данных, а не скрипт.
 */
