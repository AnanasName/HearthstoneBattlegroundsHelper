 ...
 *
 * ## ЗАМЕР ЗАКРЫТ 02.09.2026 — негоден ПО ПОСТРОЕНИЮ, а не по нехватке данных
 *
 * Контроль не взял своего порога ни разу из трёх, и третий прогон объяснил
 * почему. Смотреть надо не на «не взял», а на то, КАК числа двигались
 * с ростом выборки:
 *
 *   19 партий:  контроль 0.648 при пороге 1.170
 *   23 партии:  контроль 0.546 при пороге 0.937
 *   32 партии:  контроль 0.332 при пороге 0.674
 *
 * Среднее падает ВМЕСТЕ с порогом. Так выглядит мерка, у которой сигнала
 * нет вовсе: новые точки его не приносят, а размывают. Для сравнения,
 * у `spike:buff` на тех же трёх выборках среднее держалось (−1.27, −1.530,
 * −1.319), порог опускался ему навстречу, и на третьем прогоне замер
 * порог взял. Разница между «мало данных» и «мерка не про то» видна
 * именно в этой паре, и здесь — второе.
 *
 * Причина структурная и названа ещё в оговорках: ближайший бой чаще всего
 * не замечает лишнего тела вовсе — из 118 точек контроля разошлись 16.
 * А ценность карты, работающей ИЗ РУКИ, живёт в СЛЕДУЮЩИХ ходах (рост
 * счетовода, тройки, будущие покупки), которых эта мерка не видит
 * по построению.
 *
 * Практический вывод: **ждать от новых партий годности этого прогона
 * не надо, и перезапускать его при росте выборки бессмысленно.** Правило
 * «не советовать розыгрыш карты, работающей из руки» стоит на структурном
 * доводе (тел в бою поровну, слот тратится навсегда) и на согласии игрока,
 * а не на этих числах. Если вопрос про руку понадобится решать всерьёз,
 * нужна ДРУГАЯ мерка — с горизонтом в несколько ходов, — а не эта
 * на большей выборке. Файл оставлен как есть: он хранит метод и три
 * честных отрицательных результата.
 */
