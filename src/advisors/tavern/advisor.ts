 ...
 * игроком (docs/jeefhs.md).
 *
 * Говорит напоминание ТОЛЬКО тогда, когда есть что делать: племени
 * с 2+ своими нет — держите пару желаемого. Перечисление племён,
 * у которых пара уже набрана, убрано по жалобе игрока (20.09): оно
 * пересказывало его собственный борд, который он и так видит на экране,
 * и ни одного хода не меняло. Это та же причина, по которой из панели
 * оверлея убраны списки борда и витрины (`src/overlay/index.html`).
 *
