...
/**
 * Порог поля для ЭКРАНА — оверлея и терминала (D305), временно ниже замерного.
 *
 * Поле собирается из партий нынешнего пула (D304), и 25.09 в нём десять
 * партий, part68–part77: по 10 бордов на ходах таверны 1–11. На пороге 12
 * блок молчал везде, и игрок попросил его вернуть. Десять, а не меньше:
 * на ходах 12+ бордов 6 и меньше, и там «сильнее 3 из 5» по-прежнему
 * молчит. Сколько бордов за числом, блок печатает сам.
 *
 * Замеры, отчёт после партии и признаки фазы 6 остаются на 12: им нужна
 * калибровка, а на девяти бордах без своей партии худшая корзина — 17.3 п.п.
 * при пороге приёмки 15. Когда поле дорастёт до 12 бордов на ход, этот
 * порог возвращается к замерному.
 */
export const SCREEN_FIELD_STRENGTH_OPTIONS: FieldStrengthOptions = {
