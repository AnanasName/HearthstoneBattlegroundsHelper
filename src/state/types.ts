...
/**
 * Сколько игроков лобби живо — или `null`, если таблица не прочитана.
 *
 * Нужно симулятору для потолка урона (D288): при пяти и больше живых урон
 * боя ограничен — 5, 10 и 15 на ходах таверны 1–3, 4–7 и дальше.
 *
 * Живость решает УРОН ПРОТИВ ЗДОРОВЬЯ, а не запас с бронёй. Броня принимает
 * удар первой, поэтому у живого героя урон всегда меньше здоровья, а сдавшемуся
 * игра ставит урон ровно в здоровье и броню не трогает: part21, игрок 6 —
 * `HEALTH` 30, `DAMAGE` 30, `ARMOR` 5 с хода 13 до конца партии. Счёт по запасу
 * с бронёй давал на бою хода 22 пятерых живых при четырёх настоящих — и это
 * единственное на 873 боя корпуса «превышение» потолка (урон 22). Признак
 * живости фазы 6 (`isAlive` в src/ml) считает по запасу — он предрегистрирован
 * вместе с замером и оставлен как есть.
 *
 * Непрочитанное здоровье живым не считается. Ошибка в эту сторону безопасна:
 * меньше пяти живых снимает потолок, и смерть считается как до правки, а ошибка
 * в обратную обещала бы «не умрёте» там, где потолка нет.
 */
export function playersAlive(state: Pick<GameState, 'lobby'>): number | null {
  const players = Object.values(state.lobby);
  if (players.length === 0) return null;
  return players.filter((p) => p.health !== null && p.damage < p.health).length;
}
...
export type PlayerActionType =
  ...
  | 'levelUp'
  /**
   * Нажатие кнопки заморозки, ВКЛЮЧИВШЕЕ заморозку. Кнопка одна на оба
   * направления, и блок `PLAY` у них тождественный (part25: 14 нажатий
   * за партию, среди них снятия), а различает их то, что внутри блока:
   * витрина получает `FROZEN value=1` при заморозке и `value=0` при снятии
   * (part60, 14:35:34 и 14:35:35). До 19.09 оба писались сюда.
   */
  | 'freeze'
  /** Нажатие той же кнопки, СНЯВШЕЕ заморозку (`FROZEN value=0` в блоке). */
  | 'unfreeze'
  | 'play'
  | 'heroPower'
  | 'darkGift'
