...
export interface Minion {
  ...
  /**
   * Плейсхолдеры ВЕТВЕЙ модального «Choose One», по карте ветви.
   *
   * У ветви свои числа и своя нумерация: `BG27_084t2` пишет «+{0} Attack»
   * там, где родитель пишет «+{2} Attack», — и подстановка родительских
   * тегов давала +1 вместо +4 (part43, ход 15). Игра числа ветвей пишет
   * сама, отдельными сущностями с `CREATOR` = id этого миньона.
   *
   * Поле необязательное: у большинства миньонов ветвей нет вовсе,
   * а у части копий сущности ветвей могут не встретиться — тогда читатель
   * возвращается к соглашению «плейсхолдеры как у родителя» (part19).
   */
  readonly branchScriptData?: Readonly<Record<string, readonly (number | null)[]>>;
  /**
...
export interface Hero {
  ...
  /**
   * Тег `EXHAUSTED` на силе: `true` — жать больше нельзя, `false` — можно,
   * `null` — тега у этой силы нет вовсе.
   *
   * Три состояния, а не два, потому что тег есть НЕ У ВСЕХ сил: part8 —
   * десять нажатий и ни одного `EXHAUSTED`. Пустое значение поэтому значит
   * «игра про это молчит», и отвечать тогда обязан прежний признак
   * `heroPowerUsedThisTurn`, а не выдуманное «можно».
   *
   * Ради чего заведено: силу Инге (part45, `BG26_HERO_102p` / `p2`) игра
   * даёт нажать ДВАЖДЫ за ход, и по блокам PLAY это неотличимо от обычной
   * силы, нажатой один раз. Тег различает прямо: на первом нажатии игра
   * ставит `EXHAUSTED=1` и тут же возвращает `0` (15:27:59, строки подряд
   * внутри одного блока PLAY), на втором — `1` до конца хода, и тогда же
   * в опциях появляется `error=REQ_NOT_EXHAUSTED_HERO_POWER`. Счётчик
   * `HEROPOWER_ACTIVATIONS_THIS_TURN` за партию доходит до 2 и ни разу
   * до 3, но СКОЛЬКО нажатий положено, не написано нигде — поэтому
   * читается факт «можно ли сейчас», а не выдуманный лимит.
   */
