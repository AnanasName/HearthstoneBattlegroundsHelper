...
function marksView(input: ViewInput): OverlayMark[] {
  ...
  if (steps.length >= 2) {
    // На столе помечается ТОЛЬКО первый шаг, а не весь план сразу.
    //
    // Просьба игрока по part46 дословно: «сейчас ходы показываются все
    // на экране сразу, давай лучше сделаем последовательность — сначала
    // обводка карты, которую надо сыграть, и последовательно другие».
    // Ход на пять шагов давал до девяти колец разом (у шага бывают ещё
    // жертва, цель и носитель), и стол переставал читаться как стол.
    //
    // Последовательность получается САМА и не требует ни таймера, ни
    // анимации: шаг сделан — лог это показывает, советник пересчитывает ход
    // на новом состоянии, и первым шагом становится следующий. То есть
    // кольцо переезжает по факту сыгранного, а не по нашей догадке о том,
    // когда игрок успел. Мигающая подсветка «1 → 2 → 3» была вторым
    // вариантом и отвергнута игроком: на экране игры движение спорит
    // с самой игрой.
    //
    // Номер шага при этом ОСТАЁТСЯ: он связывает кольцо со списком в панели,
    // где ход по-прежнему виден целиком, и отличает шаг плана от колец цели
    // и жертвы, у которых номера нет.
    //
    // Помечается первый шаг, который ВИДЕН НА СТОЛЕ, а не буквально первый:
    // разыгрываемая из руки карта места на экране не имеет (рука лежит
    // веером, её геометрия не замерена), и на таком ходу стол остался бы
    // пустым при живом плане в панели. Тогда кольцо получает ближайший
    // размещаемый шаг — со СВОИМ номером, то есть игрок видит и то, что
    // помеченное действие не первое.
    for (const [i, s] of steps.entries()) {
      const before = marks.length;
      apply(s.recommendation, i + 1);
...
function tempoView(input: ViewInput, rules: TavernRules = DEFAULT_TAVERN_RULES): OverlayTempo | null {
  ...
  return {
    ...
    // мне что-то рекомендует? если да, то это неочевидно». Блок и правда
    // ничего не советует НАМЕРЕННО (замер `spike:level` вердикта не выносит),
    // но пока об этом сказано не было, картинка со шкалой и рамкой читалась
    // как указание — молча и неверно. Та же формула, что у прогноза места:
