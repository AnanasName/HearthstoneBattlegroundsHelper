...
function main(): void {
  ...
  if (data.recordsByBuild.length > 1) {
    ...
    for (const row of data.recordsByBuild) {
      const mark = row.build === data.build ? ' ← учится' : ' (прошлый патч, отброшен)';
      console.log(`  билд ${String(row.build ?? 'неизвестен')}: записей ${String(row.records)}${mark}`);
    }
    console.log(
      'Смена билда обнуляет накопление намеренно: правила и пул карт меняются ' +
        'с патчем, и учиться на прошлом — учиться другой игре (у калибровки ' +
        'это стоило 3.9 → 9.0 п.п. без единой правки кода).',
    );
    console.log('');
  }
