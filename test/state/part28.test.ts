...
describe('part28: ветвь модального миньона и предел золота', () => {
  ...
  it('предел золота у заклинания витрины: Strike Oil перестал быть невидимым', () => {
    ...
    // Третий ход таверны: впереди 9.6 хода, чистыми 7.6 золота по курсу 3.
    // Число из таблицы горизонта; было 9.3, пока она считалась по 25 партиям.
    expect(advice[0]?.score).toBeCloseTo((1 * 9.6 - 2) * 3, 5);
  ...
  it('горизонт партии: таблица замера, за её концом — ноль', () => {
    ...
    //
    // Числа пересчитаны 22.09 по 65 партиям датасета (прежние — по 25),
    // и таблица при этом УДЛИНИЛАСЬ с 15 значений до 18: партии, доживающие
    // до 16–18-го хода таверны, в прежней выборке почти не встречались.
    // Поэтому «конец таблицы» переехал с 15-го хода таверны на 18-й.
    expect(at(1)).toBe(11.6);
    expect(at(13)).toBe(5.6); // 7-й ход таверны — момент скриншота part28
    expect(at(29)).toBe(0.5); // 15-й ход таверны — уже не конец, а хвост
    expect(at(35)).toBe(0); // 18-й ход таверны — конец таблицы
    expect(at(99)).toBe(0);
  });
