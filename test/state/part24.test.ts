...
describe('part24: тиры дара, золото в плане, поглощение витрины', () => {
  ...
  beforeAll(() => {
    ...
    for (const event of readPowerEvents(text)) {
      ...
      // и ЕЩЁ НЕ заморожена — игрок заморозил её сам позже в том же ходу,
      // а на замороженной витрине правило молчит по определению. Прежний
      // селектор брал последнее состояние хода и проверял ровно его:
      // тест «на втором тире заморозки нет» был зелен на витрине, которую
      // игрок уже заморозил (та самая ловушка из test/snapshots.ts).
      if (s.turn === 3 && s.gold === 0 && s.techLevel === 2 && !s.shop.some((m) => m.frozen)) {
        turn3AfterUpgrade = s;
      }
   ...
   *
   * ЗАКРЫТ 26.08 (part27) — и не разворотом планки, а починкой её счёта.
   *
   * Здесь тест пришпиливал молчание нарочно: правка «свежая карта считается
   * по тирам витрины» опустила планку с 10.4 до 9.79, а заклинание стоило
   * 8.5, и опускать планку ещё значило вернуть совет, на который тот же
   * игрок жаловался в part22 и part23. Разбор part27 нашёл в планке ошибку,
   * а не порог: цена заморозки бралась по СРЕДНЕЙ доживающей карте, тогда
   * как игрок покупает лучшую, а пустые слоты замороженной витрины игра
   * ДОЗАПОЛНЯЕТ свежими — и подъём таверны добавляет такой слот сам
   * (в этой партии ход 3→5: три замороженных, витрина второго тира
   * из четырёх, четвёртым пришёл Soul Rewinder). Первую покупку следующего
   * хода игрок делает из свежего слота по цене свежей карты, недобор
   * платить нечем, планка равна свежей карте: 6.81 против 8.48
   * у заклинания — совет есть. Третий тир по-прежнему молчит (part23,
   * ход 11) — то самое «работает для ранней игры».
   *
   * Попутно тест перестал быть пустым: прежний селектор брал ПОСЛЕДНЕЕ
   * состояние хода 3, где витрина уже заморожена самим игроком и правило
   * молчит по определению.
   */
  it('пункт 1 (ЗАКРЫТ part27): заморозка советуется и на первом тире, и на втором после подъёма', () => {
    expect(turn1).not.toBeNull();
    expect(turn3AfterUpgrade).not.toBeNull();
    ...
    // Ход 3, та же витрина и то же заклинание, таверна уже 2 — совет тот же.
    expect(turn3AfterUpgrade.shopSpells.map((x) => x.cardId)).toEqual(['BG28_504']);
    expect(turn3AfterUpgrade.shop).toHaveLength(3);
    const freeze = freezeRule(turn3AfterUpgrade, { cards });
    expect(freeze?.action).toBe('freeze');
    expect(freeze?.spellCardId).toBe('BG28_504');
  });
