...
describe('part18: продажа в план, раннее обновление, синергия по механике', () => {
  ...
  it('ход 5: продажа экономической карты входит в план (пункт 1)', () => {
    ...
    // ГРАНИЦА СДВИНУЛАСЬ 06.09.2026, и ровно так, как обещал прежний
    // текст этого теста: «если план научится продавать ради второго тела
    // без сгорания — тест обязан упасть, и правило пересмотрят,
    // а не унаследуют молча». Научился, и не от правила продажи:
    // слагаемое АКТИВАЦИИ (part44) подняло Suspicious Prisonguard 3/3
    // с 5.0 до 8.0 (тир 2.0 + статы 3.0 + активация «+3/+3» 3.0), и второе
    // тело стало дороже заклинания за 2, ради которого цепочка прежде
    // добирала остаток. План стал «Winterfinner за 3 → продать Skipper →
    // Prisonguard за 3» — дословно то, что игрок сыграл сам и о чём
    // написал в пункте 1.
    const plan = spendPlan(turn5, { cards });
    expect(plan.steps.map((s) => s.recommendation.action)).toEqual(['buy', 'sell', 'buy']);
    expect(plan.steps[1]?.recommendation.minion?.cardId).toBe('BG33_140');
