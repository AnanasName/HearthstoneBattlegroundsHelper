...
describe('part39: своя ценность прокрутки и сила героя, дающая золото', () => {
   ...
   *
   * ВАЖНО: числа этого хода переписаны 05.09 вместе с починкой точки
   * решения. Прежде тест утверждал «золото 7» — и это было следствием
   * ДЕФЕКТА: `RESOURCES_USED` уменьшается при продаже и возвращается
   * к нулю, поэтому точка уезжала за нажатие силы и продажи, показывая
   * СЕМЬ золотых при максимуме хода ТРИ. Настоящая точка решения первого
   * хода — три золотых и витрина по три, то есть ровно одна покупка.
   */
  it('ход 1: клич свинобраза срабатывает при розыгрыше — тело можно оставить', () => {
    ...
    const s = at(1);
    // Три золотых — максимум первого хода таверны; больше их на точке
    // решения не бывает физически (7 в прежней версии теста приходили
    // от броска силы и продаж, то есть из середины хода).
  ...
  /**
   * Разделение «числа списка» и «числа цепочки» держится и на верной точке:
   * бамп прокрутки считается против лучшей покупки, а `standaloneScore`
   * его не несёт. Верхним советом прокрутка тут больше не стоит (при трёх
   * золотых первым идёт сила-кубик), поэтому правило спрашивается прямо —
   * проверяется механика, а не место в списке.
   */
  it('ход 1: у прокрутки своя ценность НИЖЕ очков списка — бамп порядка внутрь плана не идёт', () => {
    const spun = adviseTavern(at(1), deps())?.recommendations.find((r) => r.action === 'spin');
    expect(spun).toBeDefined();
    expect(spun?.standaloneScore).toBeLessThanOrEqual(spun?.score ?? 0);
    // Без соперника бамп не начисляется вовсе, и оба числа совпадают —
    // это и показывает, что надбавка приходит СНАРУЖИ правила.
  ...
  /**
   * На верной точке первого хода план совпадает с ходом игрока ДОСЛОВНО:
   * журнал part39 начинает ход 1 с `heroPower` (бросок кубика), и только
   * потом идут покупки. До починки точки советник на этом ходу предлагал
   * прокрутку — потому что видел состояние ПОСЛЕ броска, с семью золотыми.
   */
  it('ход 1: план начинается силой-кубиком — дословно ход игрока', () => {
    const plan = spendPlan(at(1), deps());
    expect(plan.goldLeft).toBe(0);
    expect(plan.steps.map((s) => s.recommendation.action)).toEqual(['heroPower', 'buy']);

    const journal = end.actions.filter((a) => a.turn === 1).map((a) => a.type);
    expect(journal[0]).toBe('heroPower');
  });
  ...
  /**
   * Число открытых точек переписано 05.09 вместе с починкой точки решения,
   * и это ЕЁ прямое следствие: прежде точка хода 1 снималась уже ПОСЛЕ
   * нажатия силы (счётчик трат вернулся к нулю), и сила там числилась
   * использованной. На верных точках она открыта трижды — 1, 11 и 13, —
   * и ровно в эти ходы игрок её и жал. Кулдаун при этом по-прежнему
   * читается без нового канала: на перезарядке игра подменяет сущность.
   */
  it('кулдаун силы читается подменой сущности: открыта на ходах 1, 11 и 13', () => {
    ...
    expect(ready.map((p) => p.state.turn)).toEqual([1, 11, 13]);
  ...
  it('ход 13: сила героя идёт ПЕРВЫМ шагом плана — как игрок её и жал', () => {
    ...
    //
    // Проверяется само ПРАВИЛО, а не порядок на этих картах. После баланса
    // 251952 (снапшот от 16.09) Trench Fighter понерфили 6/6 → 3/3, третья
    // покупка перестала перевешивать тёмный дар, и цепочка «дар → две
    // покупки → сила → …» честно выигрывает по сумме (61.6 против 59.5)
    // и не жжёт монету. Равенства здесь больше нет, а тай-брейк остаётся:
    // без доступного дара сила героя снова первая.
    const noGift = { ...at(13), darkGiftCost: null, darkGiftCharges: null };
    const withoutGift = spendPlan(noGift, deps()).steps.map((s) => s.recommendation.action);
    expect(withoutGift[0]).toBe('heroPower');
  });

  it('на перезарядке сила не советуется — гарды part13 и part37 работают без нового канала', () => {
    // Ходы, где сила подменена сущностью перезарядки (`LOCK_VISUAL`,
    // без цены и активности): правило обязано молчать на всех.
    for (const turn of [3, 5, 7, 9, 15, 17, 19]) {
      expect(at(turn).hero?.heroPowerCardId).toBe(LUCKY_ROLL_CD);
      expect(heroPowerGoldRule(at(turn), deps())).toBeNull();
    }
    // А на открытых ходах — говорит, и это та же функция.
    for (const turn of [1, 11, 13]) {
      expect(heroPowerGoldRule(at(turn), deps())).not.toBeNull();
    }
  });
