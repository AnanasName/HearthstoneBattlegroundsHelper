...
describe('part31: цена придержанного заряда дара, миньон в руку на полном борде', () => {
  ...
  it('пункт 1: на скриншоте дар уступает покупке и называет цену спешки', () => {
    ...
    // Тело тира 4; на 10-м ходу таверны (впереди 5.6 хода при трёх
    // зарядах) дар даёт тир 5–6, и разница — цена спешки. Само число
    // считается ожиданием ПО ПУЛУ и потому двигается с каждым обновлением
    // снапшота: 9.2 до баланса 251952, 9.8 после, 8.9 со снапшотом 24.09
    // (ротация пула 22.09). Проверяется не оно,
    // а вывод — дар остаётся ниже покупки и зовёт придержать заряд.
    //
    // Горизонт «5.6» — из таблицы, пересчитанной 22.09 по 65 партиям
    // (было 5.3 по 25). Замер батареей показал, что на выбор это нигде
    // не влияет, но в ПРИЧИНЕ число печатается вслух и потому видно здесь.
    expect(rec?.score).toBeCloseTo(8.9, 0);
    expect(rec?.reason).toContain('заряд лучше придержать');
    expect(rec?.reason).toContain('впереди ещё 5.6 ходов таверны');
  ...
  it('пункт 2: на неполном борде тот же росток остаётся дешёвым телом, а не молчит', () => {
    ...
    // Среднее тира 1 ПО ПУЛУ: 7.1 до снапшота 24.09, 6.4 после (ротация 22.09).
    expect(rec?.score).toBeCloseTo(6.4, 0);
