...
describe('part37: замок силы героя по тиру, цель обновления в строке действия', () => {
  ...
  /**
   * Пункт 3. Прав оказался игрок, и понято это только на part57 (17.09):
   * бесплатные обновления этого хода — ЗАПАС двух Leaf Through the Pages,
   * а запас переходит на следующий ход (`freeRefreshes`: кнопка хода 23
   * пришла с остатком 2, оба потрачены там, на золоте). Потраченное при нуле
   * золота, обновление отнимает взгляд у хода, где находку можно купить.
   */
  it('ход 21: запасное бесплатное обновление при нуле золота не советуется', () => {
  ...
  /**
   * Прежнее правило (D025) для бесплатного обновления БЕЗ запаса: на экране
   * от него не оставалось ничего, `reason` оверлей не показывает, — и цель
   * названа прямо в строке действия. Тот же кадр с `freeRefreshes: 0`.
   */
  it('ход 21: обновление без запаса при нуле золота названо целью прямо в строке действия', () => {
    const s: GameState = { ...must(shot21, 'скриншот хода 21'), freeRefreshes: 0 };
    expect(s.gold).toBe(0);
