 ...
 *     копию Hired Mount». Тогда дефекта в правиле не нашли; 17.09 (part57)
 *     выяснилось, что прав игрок: бесплатные обновления хода 21 — ЗАПАС
 *     Leaf Through the Pages, и он переходит на следующий ход. Игрок так
 *     и сыграл: на ходу 23 прокрутил витрину запасом, уже с золотом, купил
 *     третью копию `BG36_240` и собрал золотого Hired Mount 313/197.
 *     Совет с запасом теперь молчит; звенья «нашлась — заморозить,
 *     с золотом — купить» тест держит по-прежнему.
 */
describe("part38: прокрутка генератора со счётом ЦИФРОЙ, цель обновления доводится до конца", () => {
  ...
  /**
   * Пункт 2. Запас бесплатных обновлений (2 от Leaf Through the Pages) ждёт
   * хода с золотом — игрок так и сыграл на ходу 23. Без запаса действует
   * прежняя ветка: обновление называет цель, найденная копия морозится,
   * а на золоте покупается. Жалоба была на обрыв цепочки, и обрыв ловится
   * тестом.
   */
  it("ход 21: запасное обновление ждёт золота, а найденная копия доводится до тройки", () => {
    const s = must(shot21, "скриншот хода 21");
    expect(s.rerollCost).toBe(0);
    expect(s.gold).toBe(0);
    expect(s.freeRefreshes).toBe(2);
    ...
    expect(topAdvice(s).action).not.toBe("reroll");

    // Звено один — у бесплатного обновления без запаса: цель названа.
    const top = topAdvice({ ...s, freeRefreshes: 0 });
