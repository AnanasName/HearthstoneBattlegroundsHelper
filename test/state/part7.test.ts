...
describe('part7: Double Time собирает золотого из двух копий', () => {
  ...
  it('все золотые партии собрались из ДВУХ копий', () => {
    ...
    // Третий золотой партии — `BG29_503_G` на ходу 15 — сборкой НЕ БЫЛ:
    // его сделал золотым ТЁМНЫЙ ДАР (20:30:30, `DARK_GIFT_ENTITY`),
    // и простых копий до него ноль. Видно это стало только с чтением
    // `CHANGE_ENTITY` (part46): прежде строка не разбиралась вовсе,
    // блок тегов новой карты (`PREMIUM`, `ATK`, `HEALTH`) уходил
    // к ПРЕЖНЕЙ сущности, и превращение читалось как ещё одна сборка.
    // Вывод партии от этого не меняется, а становится чище: обе
    // настоящие сборки дали золотого ровно из ДВУХ копий.
    const gift = goldens.find((g) => g.cardId === 'BG29_503');
    expect(gift?.plainBefore).toBe(0);
  });
