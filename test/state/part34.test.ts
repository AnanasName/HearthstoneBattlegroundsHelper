...
describe('part34: сила «после четырёх кличевых покупок — Бранн» и Бранн вторым в расстановке', () => {
  ...
  it('ход 1: Southsea Busker (клич) встаёт над Risen Rider — прежде было 4.0 против 6.0', () => {
    ...
    // Бранн на пустом борде: тир 5 → 10, тело 2/4 → 3 и удвоение будущих
    // кличей (D280) — 6.57 = `heroPowerSpellValue` 6 × 24.1 будущих покупок
    // × 4.5 % пула первого тира, чей клич приносит карту. Всего 19.57,
    // четверть — 4.893. Доля пула — со снапшота 24.09 (ротация 22.09; было
    // 4.2 % и четверть 4.75625). Прежде награда стоила 13 и четверть 3.25: покупка
    // самого удвоителя не знала, что он удвоит.
    // Число зависит от таблицы `remainingTavernBuys` — при её перезамере
    // пересчитывать здесь, а не подгонять допуск.
    expect(b.heroPowerBuy).toBeCloseTo(4.893182, 5);
  ...
  it('ход 3: Busker 13.3 над золотой Laureate 11.5; ход 5 — Busker с долей 7.3', () => {
    ...
    // Половина награды: Бранн при двух своих кличевых стоит 14.5 телом
    // и связью плюс 6.3 за удвоение будущих кличей (6 × 23.2 покупки
    // × 4.5 % пула первого тира, D280) — всего 20.8 (до снапшота 24.09 —
    // 4.2 % и 20.3).
    expect(v5.heroPowerBuy).toBeCloseTo(10.413636, 5);
   ...
   * все три кличевых кандидата витрины несут +24.70 (16.0 телом и связью
   * при двух кличевых своих плюс 8.70 за удвоение будущих кличей —
   * 6 × 22.1 покупки × 6.6 % пула тиров 1–2, D280; до снапшота 24.09 —
   * 9.2 % и 28.24: среди ушедших ротацией 22.09 — Shell Collector), и совет говорит
   * «эта покупка приносит Brann Bronzebeard». После неё счётчик ноль,
   * и слагаемого больше нет.
   */
  it('ход 7: последняя кличевая покупка стоит целого Бранна, после неё — ноль', () => {
    const s = decision(7);
    for (const m of s.shop) {
      const v = minionValue(m, s, { cards });
      expect(v.heroPowerBuy).toBeCloseTo(24.695082, 5);
