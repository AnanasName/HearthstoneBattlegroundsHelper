...
describe('part34: сила «после четырёх кличевых покупок — Бранн» и Бранн вторым в расстановке', () => {
  ...
  it('ход 1: Southsea Busker (клич) встаёт над Risen Rider — прежде было 4.0 против 6.0', () => {
    ...
    // кличей (D280) — 6.025 = `heroPowerSpellValue` 6 × 24.1 будущих покупок
    // × 4.2 % пула первого тира, чей клич приносит карту. Всего 19.025,
    // четверть — 4.75625. Прежде награда стоила 13 и четверть 3.25: покупка
    ...
    expect(b.heroPowerBuy).toBeCloseTo(4.75625, 5);
  ...
  it('ход 3: Busker 13.3 над золотой Laureate 11.5; ход 5 — Busker с долей 7.3', () => {
    ...
    // и связью плюс 5.8 за удвоение будущих кличей (6 × 23.2 покупки
    // × 4.2 % пула первого тира, D280) — всего 20.3.
    expect(v5.heroPowerBuy).toBeCloseTo(10.15, 5);
   ...
   * все три кличевых кандидата витрины несут +28.24 (16.0 телом и связью
   * при двух кличевых своих плюс 12.24 за удвоение будущих кличей —
   * 6 × 22.1 покупки × 9.2 % пула тиров 1–2, D280), и совет говорит
  ...
  it('ход 7: последняя кличевая покупка стоит целого Бранна, после неё — ноль', () => {
    ...
    for (const m of s.shop) {
      ...
      expect(v.heroPowerBuy).toBeCloseTo(28.24, 5);
