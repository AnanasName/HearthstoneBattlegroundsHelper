...
describe('part47: сила героя, поднимающая карту витрины на тир', () => {
   ...
   * второй раз.
   *
   * **Проверка переехала с живой витрины хода 17 на витрину без Glowing
   * Cinder, и вот почему.** С D281 карта, чей эффект копится и достаётся
   * ещё не призванным телам, не идёт в жертвы на полном борде — а Glowing
   * Cinder `BG36_521` («Deathrattle: Your Elementals give an extra
   * +{0} Health this game») ровно такая. До D281 её покупка на полный
   * борд была самоубийственной: тело 4/1 тут же становилось слабейшим
   * и уходило жертвой следующей покупки, поэтому нажатие выигрывало.
   * Теперь советник покупает её сам, и нажатие из плана этого хода ушло.
   *
   * **Флип замерен и приборами не разделяется.** Он ЕДИНСТВЕННЫЙ за всю
   * партию: остальные девять планов part47 совпали до шага. Поле шести
   * виденных бордов на конце хода: 99.3 % побед и урон 22.36 у прежней
   * линии против 99.7 % и 23.03 у новой, при двух лишних золотых у новой.
   * Батарея на 52 партиях (коммит `66780c9`) — `validate:spend` не
   * шелохнулся ни одной из 16 метрик. То есть худшим новый ход не стал,
   * но и лучшим его назвать нечем: разделить их нашими мерами нельзя.
   *
   * Поэтому свойство `refreshSpend` проверяется там, где нажатие с планом
   * не конкурирует, — иначе правило осталось бы без сторожа вовсе.
   */
  it('ход 17: нажатие входит в план и обрывает его', () => {
    ...
    // силу не жал.
    const live = { ...decisionPoint(17), trinketOffer: [] };
    const state = {
      ...live,
      shop: live.shop.filter((m) => cards.info(m.cardId)?.name !== 'Glowing Cinder'),
    };
  ...
  /**
   * Сам флип D281 закреплён здесь, чтобы смена поведения не растворилась
   * в правке теста выше: на живой витрине хода 17 план ПОКУПАЕТ Glowing
   * Cinder, а нажатия в нём нет.
   */
  it('ход 17 на живой витрине: план покупает Glowing Cinder вместо нажатия (D281)', () => {
    const state = { ...decisionPoint(17), trinketOffer: [] };
    const plan = spendPlan(state, { cards });
    const first = plan.steps[0]?.recommendation;
    expect(first?.action).toBe('buy');
    expect(cards.info(first?.minion?.cardId ?? '')?.name).toBe('Glowing Cinder');
    expect(plan.steps.some((s) => s.recommendation.action === 'heroPower')).toBe(false);
  });
