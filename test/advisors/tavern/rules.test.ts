...
describe('правило обновления витрины', () => {
  ...
  it('когда найденное не на что купить, обновление советуется только под названную цель заморозки', () => {
    ...
    // «Не на что купить» — по цене, на которую можно рассчитывать, а не
    // по самой редкой в таверне. Два золота — это заклинание витрины,
    // и обновление советуется как прежде (part12, ход 19: два золота,
    // бесплатное обновление, игрок обновил и купил заклинание).
    const spare = rerollRule(state({ gold: 2, shop, rerollCost: 0, board: full }), deps);
    expect(spare?.action).toBe('reroll');
    expect(spare?.reason).toContain('покупать нечего');

    // А одно золото — нет: покупка за 1 нашлась бы в 15% витрин шестого
    // тира (замер 22.09, 947 точек датасета), и обещать её нечем
    // (part68, ход 29: три миньона по 3 и ни одного заклинания).
    expect(rerollRule(state({ gold: 1, shop, rerollCost: 0, board: full }), deps)).toBeNull();

    // Запас бесплатных обновлений на этом золоте снова под защитой D244:
    // даже названная цель заморозки его не тратит.
    expect(
      rerollRule(
        state({ gold: 1, shop, rerollCost: 0, freeRefreshes: 2, board: paired }),
        deps,
      ),
    ).toBeNull();
  });
...
describe('напоминание о тринкетах за ход до предложения (тьюторинг, JeefHS)', () => {
  it('молчит, когда пара своих уже набрана', () => {
    ...
    // но перечислять набранные племена — пересказывать борд, который игрок
    // видит сам (жалоба 20.09): говорить есть о чём только без пары.
    const s = state({
      turn: 9,
      board: [shopMinion(1, 'MURLOC_1'), shopMinion(2, 'MURLOC_2'), shopMinion(3, 'DRAGON_1')],
    });
    expect(trinketForecast(s, deps)).toBeNull();
  });

  it('амальгама парой не считается', () => {
    // Амальгама «своя» всем племенам сразу, внятного сигнала борда в ней нет:
    // мурлок с амальгамой — это по-прежнему «пары нет».
    const s = state({
      turn: 9,
      board: [shopMinion(1, 'MURLOC_1'), shopMinion(2, 'AMALGAM')],
    });
