...
describe('советник расстановки на фикстурах', () => {
  ...
  it(
    ...
    () => {
      ...
      // Качество поиска проверяется без часов: отбор режется бюджетом
      // в миллисекундах, и под нагрузкой полного прогона он успевал меньше
      // кандидатов — совет выходил на 2.9 п.п. лучше сыгранного при пороге 5
      // (16.09.2026). Время держит тест про бюджет ТЗ ниже; здесь поиск
      // останавливает потолок кандидатов, и вход определяет ответ.
      const advice = advisePosition(episode, { simulator }, { screenBudgetMs: Infinity, budgetMs: Infinity });
  ...
  it(
    ...
    () => {
      ...
      const advice = advisePosition(episode, { simulator }, { budgetMs: Infinity });
