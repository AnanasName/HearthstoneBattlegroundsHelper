...
import { buttonRect, slotRect, stageBox, type Rect } from '../../src/overlay/layout.js';
...
describe('раскладка карт на экране', () => {
  ...
  const MEASURED_BUTTONS = {
    // Центр силы — ПО ЗЕЛЁНОМУ ДИСКУ, пойманному маской по цвету
    // (x 1486..1601, y 1175..1292 кадра 2559×1599): диск нарисован внутри
    // кнопки и с ней концентричен, то есть это такой же объективный якорь,
    // как розовый медальон тира у карты витрины (part42). Прежние (1544,
    // 1239) снимались «диск плюс толщина обода» на глаз и стояли на 5.5
    // пикселя ниже; сам обод маской не ловится — он бурый на буром столе.
    heroPower: { x: 1543.5, y: 1233.5, glow: { x0: 1486, x1: 1601, y0: 1175, y1: 1292 } },
...
describe('слой меток лежит на ЭКРАНЕ, а не на окне', () => {
  // Числа — с машины игрока (part46): экран 2560×1600 при масштабе 150 %,
  // то есть 1707×1067 DIP, а рабочая область 1707×1019 — панель задач
  // отнимает 48 DIP. Окно просит себе весь экран, Windows отдаёт рабочую
  // область, и разницу видно только по кадру живой игры.
  const DISPLAY = { x: 0, y: 0, width: 1707, height: 1067 };
  const CLAMPED = { x: 0, y: 0, width: 1707, height: 1019 };

  it('ужатое окно не ужимает стол: высота остаётся экранной', () => {
    const box = stageBox(DISPLAY, CLAMPED);
    expect(box).toEqual({ left: 0, top: 0, width: 1707, height: 1067 });

    // Ради чего всё: доля силы героя, приложенная к высоте ОКНА, промахивалась
    // мимо кнопки на 48 пикселей устройства, и промах рос сверху вниз —
    // потому его и заметили у самой нижней метки, а не у витрины.
    const power = buttonRect('heroPower', DISPLAY.width / DISPLAY.height);
    const centerOnStage = box.top + (power.y + power.h / 2) * box.height;
    const centerOnWindow = (power.y + power.h / 2) * CLAMPED.height;
    expect(Math.round(centerOnStage)).toBe(823);
    expect(Math.round(centerOnWindow)).toBe(786);
  });

  it('окно, сдвинутое от угла экрана, сдвигает и стол — на разницу углов', () => {
    // Свойство, ради которого прямоугольник считается разницей, а не
    // копируется: окно не обязано начинаться в углу экрана.
    const moved = stageBox(DISPLAY, { x: 10, y: 40, width: 1707, height: 1019 });
    expect(moved).toEqual({ left: -10, top: -40, width: 1707, height: 1067 });
  });

  it('окно во весь экран оставляет стол на месте', () => {
    // Прежнее поведение — частный случай нового, и это единственная гарантия,
    // что правка ничего не сдвинула там, где всё и так было верно.
    expect(stageBox(DISPLAY, DISPLAY)).toEqual({ left: 0, top: 0, width: 1707, height: 1067 });
  });
});
