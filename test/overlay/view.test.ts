...
describe('вид оверлея', () => {
  ...
  it('темп: ярлык прямо говорит, что это НЕ совет', () => {
    // Вопрос игрока по part43 дословно: «что означает надпись про темп? она
    // мне что-то рекомендует? если да, то это неочевидно». Блок вердикта
    // не выносит намеренно (`OverlayTempo`), и сказать это надо словами —
    // той же формулой, что у прогноза места («прогноз, не совет»).
    const view = buildView(input(), cards);
